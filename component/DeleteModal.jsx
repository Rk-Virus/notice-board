...
import { deleteNotice } from '@/utils/ApiFunctions'

const DeleteModal = ({ setIsOpen, noteId }) => {
    const handleDelete = async () => {
        try {
            const res = await deleteNotice(noteId)
            if (res.success) {
                console.log('deleted')
                setIsOpen(false);
                location.reload(); // will remove soon
            } else {
                console.log(res?.message || 'error while deleting')
            }
        } catch (error) {
